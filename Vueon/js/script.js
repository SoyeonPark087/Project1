...
const [
  ...
  fetchList("/discover/movie", {
    ...
    heroMovie = trending.find(m=>m.backdropPath) || popular.find(m=>m.backdropPath) || popular[0];
...
function renderHero(movie){
  ...
  title.innerHTML = movie.title.replace(/\s+/g,"<br>");
  meta.innerHTML = `
    ...
    <article class="card ${rank ? "rank-card" : ""}" data-id="${movie.id}" tabindex="0" aria-label="${movie.title} 상세 정보">
      ...
      <div class="card-art tmdb-art" style="background-image:${artStyle(movie,"backdrop")}">
        <div class="card-info">
          ...
          <span>★ ${movie.rating.toFixed(1)} · ${movie.genre || "영화"}</span>
        </div>
      </div>
    </article>`;
}
